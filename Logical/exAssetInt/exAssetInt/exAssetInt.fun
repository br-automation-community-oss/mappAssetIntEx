...
FUNCTION_BLOCK Logger
	VAR_INPUT
		log_level : {REDUND_UNREPLICABLE} USINT; (*Log Level*)
		log_msg : {REDUND_UNREPLICABLE} STRING[LOG_MSG_LEN]; (*Log message*)
		log_buffer : {REDUND_UNREPLICABLE} UDINT; (*Address of log buffer*)
		log_size : {REDUND_UNREPLICABLE} UINT; (*Size of one log entry*)
		log_len : {REDUND_UNREPLICABLE} UINT; (*Number of log entries*)
	END_VAR
	VAR
		rtc_gettime : {REDUND_UNREPLICABLE} RTCtime_typ;
		tmpSTR : {REDUND_UNREPLICABLE} STRING[20];
		last_msg : {REDUND_UNREPLICABLE} STRING[LOG_MSG_LEN];
	END_VAR
END_FUNCTION_BLOCK
